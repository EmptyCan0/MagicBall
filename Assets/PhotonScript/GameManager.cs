...
public class GameManager : MonoBehaviourPunCallbacks, IPunObservable
{
    ...
    void Update()
    {
        if (ChangeCheck && !isPitcher)  //チェンジしましょうというサインが出て、バッターのときはチェンジのパネルが出るのを0.5秒遅らせる
        {
            Invoke("delayMethod", 0.5f);
            ChangeCheck = false;
        }

        //自分がピッチャーのとき、初期状態に戻せと命令が来たとき
    ...
    private void FixedUpdate()
    {
        ...
        {
            GoSign.anchoredPosition = EndPos_Of_Image;
            float AlphaChangePower = 0.05f;
            FlashPanel.color = new Color(1.0f, 1.0f, 1.0f, Mathf.Min(1.0f, FlashPanel.color.a + AlphaChangePower));
            if (FlashPanel.color.a >= 0.99f && !oneshot)
            {
                oneshot = true;
                if (!isPitcher)
                {
        ...
        {
            float AlphaChangePower_Negative = 0.97f;
            FlashPanel.color = new Color(1.0f, 1.0f, 1.0f, Mathf.Min(1.0f, FlashPanel.color.a * AlphaChangePower_Negative));
            if (FlashPanel.color.a <= 0.1f && oneshot)
            {
                oneshot = false;
                GoSign.anchoredPosition = StartPos_Of_Image;
                if (!isPitcher)
                {
    ...
    void JustDelay()
    {
        ...
        Panel_Matching.SetActive(false);
    }
